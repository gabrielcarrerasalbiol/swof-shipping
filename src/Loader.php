...
class Loader
{
    ...
    public function bootstrap(): void
    {
        add_action('before_woocommerce_init', function() {
            if (class_exists(\Automattic\WooCommerce\Utilities\FeaturesUtil::class)) {
                \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility('custom_order_tables', $this->pluginMeta->getEntryFile(), true);
            }
        });

        $this->installServices();

        add_filter('woocommerce_shipping_methods', [$this, '_registerShippingMethods']);
    ...
    public function _registerShippingMethods($shippingMethods)
    {
        ...
        if (!isset($shippingMethod)) {
            $shippingMethod = new swof_tree_table_rate();
        }

        $shippingMethods[swof_tree_table_rate::className()] = $shippingMethod;
    ...
    public function _injectSettingsLink($links)
    {
        ...
        $customLinks = array(
            $base => 'Shipping zones',
            $base . '&section=' . rawurlencode(swof_tree_table_rate::className()) => 'Global shipping rules',
        );
     ...
     */
    private static function editingShippingMethod(): ?ShippingMethod
    {
        if (isset($_GET['section']) && $_GET['section'] === swof_tree_table_rate::className()) {
        ...
        if (isset($instanceId) &&
            ...
            ($method instanceof swof_tree_table_rate)) {
    ...
    private static function removeConflictingScripts(): void
    {
    ...
    private static function createMigrationService(PluginMeta $meta): \TrsVendors\Dgm\Shengine\Migrations\MigrationService
    {
        global $wpdb;

        $options = new \TrsVendors\Dgm\Shengine\Migrations\Storage\WordpressOptions($wpdb);

        return new \TrsVendors\Dgm\Shengine\Migrations\MigrationService(
            $meta->getVersion(),
            $options->bind('swofshipping_option'),
            $meta->getMigrationsPath(),
            new ConfigStorage('woocommerce_swof_tree_table_rate_shipping_settings', $options)
        );
    }
