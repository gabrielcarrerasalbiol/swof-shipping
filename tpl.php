...
use Trs\Conditions\CouponCondition;
...
$coupons = call_user_func(static function() {
    ...
    $coupons = [CouponCondition::ANY_FREE_SHIPPING_COUPON => '[any free shipping coupon]'];
    ...
    <script id="trs_template" type="text/html">
        <div class="rules">
            {{#if !rule.children.length}}
                <?php include __DIR__.'/tpl/copypaste/paste.html' ?>
            {{/if}}
            <ul>
                <li class="root">
                    {{#with rule}}
                        {{#if .children.length}}
                            ...
                            {{#with .operations.list.0.add.calculator.children}}
                                <div class="root-options">
                                    ...
                                    <?php include __DIR__.'/tpl/copypaste/copypaste.html' ?>
                                </div>
                            {{/with}}
                    ...
                    {{#if ._view.showSettings}}
                        <div class="settings form panel" intro-outro="slide">
                            ...
                            <div class="rate-title">
                                <input type="text" placeholder="Shipping option title" value="{{.meta.title}}" size="60">
    ...
    <script id="trs_operations" type="text/html">
                        ...
                        {{#with .calculator}}
                            <select value="{{.calculator}}">
                                ...
                                {{#if children.length || .calculator == 'children'}}
                                    <option disabled class="separator">────────────────────</option>
                                    <option value="children">Nested rules</option>
                                {{/if}}
                            </select>
        ...
        {{#partial aggregator}}
            <span class="nowrap">
                ...
                <span class="helper">
                    {{#if .aggregator == 'all'}}
                        {{#if calculator == 'children'}}nested{{/if}} rates
                    {{elseif .aggregator == 'sum'}}
                        of {{#if calculator == 'children'}}nested{{/if}} rates
                    {{else}}
                        {{#if calculator == 'children'}}nested{{/if}} rate
                    {{/if}}
                </span>
            </span>
        {{/partial}}
